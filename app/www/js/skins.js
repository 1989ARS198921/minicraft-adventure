...
function drawFace(g, o) {
  ...
  if (o.style === 'skeleton') { // 💀 череп: тёмные глазницы и зубы
    P(g, X + 1, Y + 2, 2, 2, '#2A2620'); P(g, X + 5, Y + 2, 2, 2, '#2A2620'); // глазницы
    P(g, X + 1, Y + 3, 1, 1, '#88FF88'); P(g, X + 6, Y + 3, 1, 1, '#88FF88'); // зелёные огоньки
    P(g, X + 3, Y + 4, 2, 1, shade(o.skin, 0.8)); // скулы
    P(g, X + 3, Y + 5, 2, 1, '#2A2620'); // нос-дырка
    P(g, X + 1, Y + 6, 6, 1, '#2A2620'); // рот-щель
    P(g, X + 2, Y + 6, 1, 1, o.skin); P(g, X + 4, Y + 6, 1, 1, o.skin); P(g, X + 6, Y + 6, 1, 1, o.skin); // зубы
    return;
  }
  if (o.style === 'vampire') { // 🧛 бледный аристократ с клыками
    P(g, X + 1, Y + 2, 2, 1, '#0A0A0A'); P(g, X + 5, Y + 2, 2, 1, '#0A0A0A'); // тонкие брови домиком
    P(g, X + 1, Y + 3, 2, 1, '#FF1A1A'); P(g, X + 5, Y + 3, 2, 1, '#FF1A1A'); // красные глаза
    P(g, X + 2, Y + 4, 1, 1, '#600000'); P(g, X + 5, Y + 4, 1, 1, '#600000'); // тёмные зрачки
    P(g, X + 3, Y + 4, 2, 1, shade(o.skin, 0.85)); // благородный нос
    P(g, X + 2, Y + 6, 4, 1, '#5A1020'); // рот
    P(g, X + 2, Y + 6, 1, 2, '#FFFFFF'); P(g, X + 5, Y + 6, 1, 2, '#FFFFFF'); // клыки вниз!
    P(g, X + 0, Y + 5, 1, 1, shade(o.skin, 0.9)); P(g, X + 7, Y + 5, 1, 1, shade(o.skin, 0.9)); // впалые щёки
    return;
  }
  if (o.style === 'zombie') { // 🧟 мутные глаза, кривой рот, шов на лбу
    P(g, X + 1, Y + 3, 2, 2, '#C8D8B8'); P(g, X + 5, Y + 3, 2, 2, '#C8D8B8'); // мутные белки
    P(g, X + 2, Y + 4, 1, 1, '#20301A'); P(g, X + 5, Y + 4, 1, 1, '#20301A'); // вялые зрачки
    P(g, X + 3, Y + 5, 2, 1, shade(o.skin, 0.7)); // нос
    P(g, X + 2, Y + 6, 3, 1, '#1A2414'); P(g, X + 5, Y + 7, 1, 1, '#1A2414'); // кривой рот
    P(g, X + 6, Y + 2, 2, 1, '#3A4A30'); P(g, X + 6, Y + 1, 1, 1, '#3A4A30'); // шов на лбу
    return;
  }
...
function drawHair(g, o) {
  ...
  if (o.style === 'skeleton') return; // 💀 лысый череп
  if (o.style === 'vampire') { // 🧛 гладкие чёрные волосы + вдовий пик
    P(g, 8, 0, 8, 8, o.hair);            // макушка
    P(g, 8, 8, 8, 2, o.hair);            // чёлка
    P(g, 11, 10, 2, 2, o.hair);          // вдовий пик
    P(g, 0, 8, 8, 3, o.hair); P(g, 16, 8, 8, 3, o.hair); // виски
    P(g, 24, 8, 8, 4, o.hair);           // затылок
    P(g, 9, 1, 2, 1, shade(o.hair, 1.6)); // блик
    return;
  }
  if (o.style === 'zombie') { // 🧟 редкие клочья на облысевшей голове
    P(g, 9, 0, 3, 2, o.hair); P(g, 13, 1, 2, 2, o.hair);
    P(g, 24, 8, 4, 3, o.hair); P(g, 8, 8, 2, 1, o.hair);
    return;
  }
